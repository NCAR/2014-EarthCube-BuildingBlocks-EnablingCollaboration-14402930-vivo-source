...
public class ContactMailController extends FreemarkerHttpServlet {
    ...
    private void sendMessage(Session s, String webuseremail, String webusername,
        ...
        msg.setFrom(new InternetAddress(recipients[0]));
        ...
        try {
            msg.setReplyTo(new Address[]{
                    new InternetAddress(webuseremail, webusername)
            });
        } catch (UnsupportedEncodingException e) {
            log.error("Can't set message reply email with personal name " + webusername +
                    " due to UnsupportedEncodingException");
            msg.setReplyTo( new Address[]{
                    new InternetAddress(webuseremail)
            });
        }
