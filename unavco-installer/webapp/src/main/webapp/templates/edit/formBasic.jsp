...
<table cellpadding="4" cellspacing="2"  style="background-color:#fff">
	...
	<tr class="editformcell">
		<td colspan="${colspan}">
			<c:choose>
				<c:when test='${_action == "insert"}'>
					<input id="primaryAction" type="submit" class="btn btn-success submit" name="_insert" value="Create New Record"/>
				</c:when>
				<c:otherwise>
    				<input id="primaryAction" type="submit" class="btn btn-success submit" name="_update" value="Submit Changes"/>
                    <c:if test="${ ! (_cancelButtonDisabled == 'disabled') }">
				        <input type="submit" class="btn btn-danger delete" name="_delete" onclick="return confirmDelete();" value="Delete"/>
                    </c:if>
				</c:otherwise>
			</c:choose>

			<input type="reset"  class="btn btn-warning delete" value="Reset"/>

            <c:choose>
                <c:when test="${!empty formOnCancel}">
                    <input type="submit" class="btn btn-warning delete" name="_cancel" onclick="${formOnCancel}" value="Cancel"/>
                </c:when>
                <c:otherwise>
		            <input type="submit" class="btn btn-warning delete" name="_cancel" value="Cancel"/>
                </c:otherwise>
            </c:choose>
		</td>
	</tr>
