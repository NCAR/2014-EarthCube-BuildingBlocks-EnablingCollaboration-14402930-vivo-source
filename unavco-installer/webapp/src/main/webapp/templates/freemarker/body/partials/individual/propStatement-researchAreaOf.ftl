    ...
    <#local linkedIndividual>
     ...
     <#if !(statement.dateTimeEnd?has_content)>
        ...
        <#if statement.parentOrg??>
            <#local parOrgString>
                <a href="${profileUrl(statement.uri("parentOrg"))}" title="${i18n().organization_name}">${statement.parentOrgLabel!""}</a>
            </#local>
        </#if>
    </#if>
    <@s.join [ linkedIndividual, posnTitle!, orgString!, parOrgString!  ] />
</#macro>
