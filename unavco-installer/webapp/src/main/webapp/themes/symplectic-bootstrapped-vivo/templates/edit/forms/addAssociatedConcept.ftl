...
<ul class="list-unstyled" id="existingConcepts">
...
<div id="showAddForm">
    ...
    <input class="btn btn-success" type="submit" value="${i18n().add_concept}" id="showAddFormButton" name="showAddFormButton">
    <a class="btn btn-warning cancel" href="${cancelUrl}&url=/individual" title="${i18n().return_to_profile}">${i18n().return_to_profile}</a>
</div>
    ...
    <#list sources?values?sort_by("label") as thisSource>
		<div class="radio">
				<label><input type="radio"  name="source" value="${thisSource.schema}" role="radio" <#if checkedSource = false><#assign checkedSource = true/>checked="checked"</#if>>
       <a href="${thisSource.url}">${thisSource.label}</a> &nbsp;(${thisSource.description})</label>
		</div>

    </#list>
    <p class="inline-search">
        <input type="text" id="searchTerm" label="Search" class="form-control acSelector" size="35" />
        <input type="button" class="btn btn-success submit concept-search" id="searchButton" name="searchButton" value="${i18n().search_service_btn}"/>&nbsp;
    ...
    <p class="submit">
        <input class="btn btn-success" type="submit" id="submit" name="submit" value="${i18n().add_selected_concept}" />
