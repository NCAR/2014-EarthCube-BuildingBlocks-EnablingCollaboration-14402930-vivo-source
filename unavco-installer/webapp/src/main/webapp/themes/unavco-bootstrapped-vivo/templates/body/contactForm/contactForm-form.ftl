...
<section class="staticPageBackground feedbackForm" role="region">
    ...
    <form name="contact_form" id="contact_form" class="customForm" action="${formAction!}" method="post" onSubmit="return ValidateForm('contact_form');" role="contact form">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="webusername" value="${webusername!}"/>
        </div>
        <div class="form-group">
            <label for="webuseremail">${i18n().email_address} <span class="requiredHint"> *</span></label>
            <input type="text" class="form-control" name="webuseremail"  value="${webuseremail!}"/>
        </div>

        <div class="form-group">
            <label>${i18n().comments_questions} <span class="requiredHint"> *</span></label>
            <textarea name="s34gfd88p9x1" class="form-control" rows="10" cols="90">${comments!}</textarea>
        </div>

    	<div class="form-group">
        <label class="realpersonLabel">${i18n().enter_in_security_field}:<span class="requiredHint"> *</span></label>

    		<input type="text" class="form-control" id="defaultReal" name="defaultReal"></p>
      </div>
      <#--  <div class="buttons"> -->
            <br /><input id="submit" class="btn btn-success" type="submit" value="${i18n().send_mail}" />
      <#--  </div> -->
