...
<form class="form-inline customForm" action ="${submitUrl}" class="customForm">
<p></p>
<#--Need to draw edit elements for dates here-->
 ...
 <#if htmlForElements?keys?seq_contains("endField")>
    ...
    <p><span class="requiredHint"> *${i18n().datetime_year_required}</span></p>
    <p class="submit">
        <input type="hidden" name="editKey" value="${editKey}" />
        <input class="btn btn-success" type="submit" id="submit" value="${submitButtonText}" role="button" />
        <a class="btn btn-warning cancel" href="${editConfiguration.cancelUrl}" title="${i18n().cancel_title}">${i18n().cancel_link}</a>
    </p>
</form>
