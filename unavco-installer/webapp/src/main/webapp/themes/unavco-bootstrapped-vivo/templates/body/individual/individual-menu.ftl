...
<#if hasElement?has_content>
    ...
    <#-- List the menu items -->
    <ul class="menuItems list-group sortable">
        <#list hasElement.statements as statement>
            <li class="menuItem list-group-item"><#include "${hasElement.template}"> <span class="controls"><!--p.editingLinks "hasElement" "" statement editable /--></span></li>
        </#list>
    </ul>
    ...
    <#if editable>
	    <form id="pageListForm" action="${urls.base}/editRequestDispatch" method="get">
	        ...
	        <input type="hidden" name="editForm" value="edu.cornell.mannlib.vitro.webapp.edit.n3editing.configuration.generators.ManagePageGenerator" role="input">
	   		...
	   		<input class="btn btn-success" id="submit" value="Add new menu page" role="button" type="submit" >
    ...
    <#-- Sortable adds drag and drop lists to mobile -->
    ${headScripts.add('<script type="text/javascript" src="https://rawgit.com/RubaXa/Sortable/master/Sortable.js"></script>')}
