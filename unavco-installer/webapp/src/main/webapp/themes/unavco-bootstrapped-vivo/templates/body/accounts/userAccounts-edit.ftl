    ...
    <form method="POST" action="${formUrls.edit}" id="userAccountForm" class="customForm" role="edit account">
      ...
      <div class="form-group">
		...
		<#if externalAuthPermitted??>
            ...
            <div class="checkbox"><label><input id="externalAuthChkBox" type="checkbox" name="externalAuthOnly"  <#if externalAuthOnly?? >checked</#if>  />${strings.external_auth_only}</label></div>
        </#if>
        ...
        <#if roles?has_content>
            ...
            <#list roles as role>
              <div class="radio">
                  <label><input type="radio" name="role" value="${role.uri}" role="radio" ${selectedRoles?seq_contains(role.uri)?string("checked", "")} />${role.label}</label>
              </div>
            </#list>
        </#if>
        ...
        <p><input class="btn btn-success" type="submit" id="submitMyAccount" name="submitEdit" value="${strings.save_changes}" class="submit" disabled /> ${strings.or} <a class="cancel" href="${formUrls.list}" title="${strings.cancel_title}">${strings.cancel_link}</a></p>
